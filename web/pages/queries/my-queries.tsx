...
const MyQueries: NextPage<{ me: User; columns: any }> = ({ me, columns }) => {
  const router = useRouter();
  const { results, dataError, dataMsg, error } = useTable({
    getUrl: '/api/queries/my-queries',
    postUrl: '/api/queries/my-queries',
    buttonSettings: {
      isPopover: false,
      text: 'Delete Query',
      color: 'error',
...
export const getServerSideProps = withSsrSession(async function ({
  ...
  try {
    ...
    console.log(`[/QUERIES/COMPLIANCE] ${error}`);
  }
